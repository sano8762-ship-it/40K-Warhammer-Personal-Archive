---
標題: 始祖鳥矢量飛梭
英文名稱: Archaeopter Transvector
陣營: 機械修會
來源版本: 使用者提供《機械修會11版中文1.0.pdf》；Faction Pack 1.1；Munitorum Field Manual v1.2
更新日期: 2026-08-19
---

# 始祖鳥矢量飛梭（Archaeopter Transvector）
## 官方產品圖片

![始祖鳥矢量飛梭｜Archaeopter Transvector｜Games Workshop 官方商品圖](../../../../assets/官方商店/機械修會/Adeptus-Mechanicus-Archaeopter-2020.jpg)

> **圖片來源：** [Games Workshop 官方商店](https://www.warhammer.com/en-WW/shop/Adeptus-Mechanicus-Archaeopter-2020)。官方商品主圖。
> **圖片擷取日期：** 2026-08-22。


本頁依使用者提供的《機械修會11版中文1.0.pdf》建立，並以官方 Munitorum Field Manual v1.2 的現行點數為準。規則關鍵字採繁體中文；英文名稱僅保留作為原文索引。

## 單位數據

| M | T | SV | W | LD | OC | 特殊保護 |
|---:|---:|---:|---:|---:|---:|---:|
| 14 | 9 | 3+ | 10 | 7+ | 0 | — |

## 能力

**陣營：機神律令。** 本模型可受機神律令影響。

**核心：致命破滅 D3、懸浮、深入打擊。** 本模型擁有致命破滅 D3、懸浮與深入打擊。

**空降投送。** 在你的第一個移動階段，本模型可以進行入場移動。

**嚴重損傷。** 本模型傷口為 1–3 時，攻擊命中結果減 1。

## 武器

| 武器 | 射程 | A | BS／WS | S | AP | D | 能力 |
|---|---:|---:|---:|---:|---:|---:|---|
| 智能重機槍陣列 | 36 | 9 | 4+ | 4 | 0 | 1 | 速射 9、連擊 1、雙聯 |
| 裝甲機體 | 近戰 | 3 | 4+ | 6 | 0 | 1 | 無 |

## 編制、點數與裝備

| 項目 | 內容 |
|---|---|
| 編制 | 1 架始祖鳥矢量飛梭。 |
| 點數 | 145 分 |
| 裝備 | 智能重機槍陣列、裝甲機體、指揮鏈路。 |
| 升級選項 | 指揮鏈路可換為幹擾彈發射器，使本模型獲得煙霧彈。 |
| 版本註記 | 可運載最多 11 個護教軍步兵或技術牧師步兵模型；不能運載跳躍揹包、武裝奴工或西多尼安哨兵。Faction Pack 1.1 已移除飛行器關鍵字並將移動改為 14；本頁沿用 PDF 的懸浮與深入打擊資料表標示，規則更新頁的官方修訂優先；點數依官方 Munitorum Field Manual v1.2。 |

## 關鍵字

**關鍵字：** 載具、飛行、帝國、護教軍、始祖鳥矢量飛梭。

**陣營關鍵字：** 機械修會。

## 資料來源

本頁規則資料依使用者提供的《機械修會11版中文1.0.pdf》整理，並套用官方 Faction Pack 1.1 更新；點數依官方 Munitorum Field Manual v1.2。

## References

[1]: https://mfm.warhammer-community.com/en/adeptus-mechanicus "Munitorum Field Manual｜Adeptus Mechanicus v1.2"
[2]: https://assets.warhammer-community.com/eng_22-07_warhammer_40,000_faction_pack_adeptus_mechanicus-d1ubc1apog-mpt3r8xzy4.pdf "Faction Pack: Adeptus Mechanicus Version 1.1"
