---
標題: 天蠍座沙丘行者運輸船
英文名稱: Skorpius Dunerider
陣營: 機械修會
來源版本: 使用者提供《機械修會11版中文1.0.pdf》；Faction Pack 1.1；Munitorum Field Manual v1.2
更新日期: 2026-08-19
---

# 天蠍座沙丘行者運輸船（Skorpius Dunerider）
## 官方產品圖片

![天蠍座沙丘行者運輸船｜Skorpius Dunerider｜Games Workshop 官方商品圖](../../../../assets/官方商店/機械修會/Adeptus-Mechanicus-Skorpius-Dunerider-2019.jpg)

> **圖片來源：** [Games Workshop 官方商店](https://www.warhammer.com/en-WW/shop/Adeptus-Mechanicus-Skorpius-Dunerider-2019)。官方商品主圖。
> **圖片擷取日期：** 2026-08-22。


本頁依使用者提供的《機械修會11版中文1.0.pdf》建立，並以官方 Munitorum Field Manual v1.2 的現行點數為準。規則關鍵字採繁體中文；英文名稱僅保留作為原文索引。

## 單位數據

| M | T | SV | W | LD | OC | 特殊保護 |
|---:|---:|---:|---:|---:|---:|---:|
| 12 | 9 | 3+ | 11 | 7+ | 2 | — |

## 能力

**陣營：機神律令。** 本模型可受機神律令影響。

**核心：致命破滅 D3、開火口 2。** 本模型擁有致命破滅 D3 與開火口 2。

**廣域數據鏈。** 本模型成為你的策略目標時，擲 D6；5+ 時你獲得 1 CP。

**火力支援。** 射擊完成後，選擇本模型命中的一個敵方單位；本階段從本模型下車的友軍單位攻擊該敵方單位時，可重擲造傷結果。

## 武器

| 武器 | 射程 | A | BS／WS | S | AP | D | 能力 |
|---|---:|---:|---:|---:|---:|---:|---|
| 智能重機槍陣列 | 36 | 9 | 4+ | 4 | 0 | 1 | 速射 9、連擊 1、雙聯 |
| 裝甲車體 | 近戰 | 3 | 4+ | 6 | 0 | 1 | 無 |

## 編制、點數與裝備

| 項目 | 內容 |
|---|---|
| 編制 | 1 臺天蠍座沙丘行者運輸船。 |
| 點數 | 75 分；第四個以上 85 分 |
| 裝備 | 智能重機槍陣列、裝甲車體。 |
| 升級選項 | 無。 |
| 版本註記 | 可運載最多 12 個機械修會步兵模型；不能運載擁有跳躍揹包、武裝奴工或西多尼安哨兵關鍵字的模型；點數依官方 Munitorum Field Manual v1.2。 |

## 關鍵字

**關鍵字：** 載具、運輸工具、專屬運輸、帝國、護教軍、煙霧彈、模型框架、天蠍座沙丘行者。

**陣營關鍵字：** 機械修會。

## 資料來源

本頁規則資料依使用者提供的《機械修會11版中文1.0.pdf》整理，並套用官方 Faction Pack 1.1 更新；點數依官方 Munitorum Field Manual v1.2。

## References

[1]: https://mfm.warhammer-community.com/en/adeptus-mechanicus "Munitorum Field Manual｜Adeptus Mechanicus v1.2"
[2]: https://assets.warhammer-community.com/eng_22-07_warhammer_40,000_faction_pack_adeptus_mechanicus-d1ubc1apog-mpt3r8xzy4.pdf "Faction Pack: Adeptus Mechanicus Version 1.1"
