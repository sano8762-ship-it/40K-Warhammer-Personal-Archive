---
標題: 護教軍先鋒兵
英文名稱: Skitarii Vanguard
陣營: 機械修會
來源版本: 使用者提供《機械修會11版中文1.0.pdf》；Faction Pack 1.1；Munitorum Field Manual v1.2
更新日期: 2026-08-19
---

# 護教軍先鋒兵（Skitarii Vanguard）
## 官方產品圖片

![護教軍先鋒兵｜Skitarii Vanguard｜Games Workshop 官方商品圖](../../../../其他/圖片/帝國軍/機械修會/Skitarii-Vanguard-2017.jpg)

> **圖片來源：** [Games Workshop 官方商店](https://www.warhammer.com/en-WW/shop/Skitarii-Vanguard-2017)。官方商品主圖。
> **圖片擷取日期：** 2026-08-22。


本頁依使用者提供的《機械修會11版中文1.0.pdf》建立，並以官方 Munitorum Field Manual v1.2 的現行點數為準。規則關鍵字採繁體中文；英文名稱僅保留作為原文索引。

## 單位數據

| M | T | SV | W | LD | OC | 特殊保護 |
|---:|---:|---:|---:|---:|---:|---:|
| 6 | 3 | 4+ | 1 | 7+ | 2 | 5+ |

## 能力

**陣營：機神律令。** 本單位可受機神律令影響。

**輻射污染。** 本單位 3 吋內敵方非載具單位的目標控制減 1。

## 武器

| 武器 | 射程 | A | BS／WS | S | AP | D | 能力 |
|---|---:|---:|---:|---:|---:|---:|---|
| 機械教手槍 | 12 | 1 | 4+ | 6 | -1 | 1 | 毀滅傷害、手槍 |
| 電弧步槍 | 30 | 1 | 4+ | 8 | -1 | D3 | 反載具 4+、毀滅傷害、速射 1 |
| 輻射卡賓槍 | 18 | 3 | 4+ | 3 | 0 | 1 | 反步兵 4+ |
| 超鈾元素步槍 | 36 | 1 | 4+ | 7 | -2 | D3 | 重型、精準 |
| 等離子連發槍：標準 | 30 | 2 | 4+ | 7 | -2 | 1 | 無 |
| 等離子連發槍：過載 | 30 | 2 | 4+ | 8 | -3 | 2 | 危險 |
| 隊長格鬥武器 | 近戰 | 2 | 4+ | 5 | -1 | 1 | 無 |
| 格鬥武器 | 近戰 | 1 | 4+ | 3 | 0 | 1 | 無 |

## 編制、點數與裝備

| 項目 | 內容 |
|---|---|
| 編制 | 1 個先鋒兵隊長與 9 個先鋒兵。 |
| 點數 | 85 分 |
| 裝備 | 輻射卡賓槍、格鬥武器。 |
| 升級選項 | 隊長可額外裝備隊長格鬥武器或將輻射卡賓槍換為機械教手槍；先鋒兵可各自將輻射卡賓槍換為電弧步槍、等離子連發槍或超鈾元素步槍；最多一名模型可裝備強化數據鏈或占卜儀。 |
| 版本註記 | 點數依 MFM v1.2：85 分；PDF 原始編排中的點數差異以 MFM v1.2 為準 |

## 關鍵字

**關鍵字：** 步兵、戰線、帝國、護教軍、先鋒兵。

**陣營關鍵字：** 機械修會。

## 資料來源

本頁規則資料依使用者提供的《機械修會11版中文1.0.pdf》整理，並套用官方 Faction Pack 1.1 更新；點數依官方 Munitorum Field Manual v1.2。

## References

[1]: https://mfm.warhammer-community.com/en/adeptus-mechanicus "Munitorum Field Manual｜Adeptus Mechanicus v1.2"
[2]: https://assets.warhammer-community.com/eng_22-07_warhammer_40,000_faction_pack_adeptus_mechanicus-d1ubc1apog-mpt3r8xzy4.pdf "Faction Pack: Adeptus Mechanicus Version 1.1"
