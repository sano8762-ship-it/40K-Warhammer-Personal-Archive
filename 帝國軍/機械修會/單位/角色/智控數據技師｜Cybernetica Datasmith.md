---
標題: 智控數據技師
英文名稱: Cybernetica Datasmith
陣營: 機械修會
來源版本: 使用者提供《機械修會11版中文1.0.pdf》；Faction Pack 1.1；Munitorum Field Manual v1.2
更新日期: 2026-08-19
---

# 智控數據技師（Cybernetica Datasmith）
## 官方產品圖片

![智控數據技師｜Cybernetica Datasmith｜Games Workshop 官方商品圖](../../../../其他/圖片/帝國軍/機械修會/Kastelan-Robots-2017.jpg)

> **圖片來源：** [Games Workshop 官方商店](https://www.warhammer.com/en-WW/shop/Kastelan-Robots-2017)。智控數據技師為卡斯特蘭機器人套件的隨附模型，此處使用同一官方套件商品圖作為辨識參考。
> **圖片擷取日期：** 2026-08-22。


本頁依使用者提供的《機械修會11版中文1.0.pdf》建立，並以官方 Munitorum Field Manual v1.2 的現行點數為準。規則關鍵字採繁體中文；英文名稱僅保留作為原文索引。

## 單位數據

| M | T | SV | W | LD | OC | 特殊保護 |
|---:|---:|---:|---:|---:|---:|---:|
| 6 | 4 | 2+ | 3 | 7+ | 1 | 5+ |

## 能力

**陣營：機神律令。** 本模型可受機神律令影響。

**核心：支援。** Faction Pack 1.1 已移除本模型的領袖能力，並改為支援；本模型不再因無法加入卡斯特蘭機器人而自動被消滅。

**數據斷鏈。** 如果本單位不包含任何卡斯特蘭機器人模型，則本單位失去載具關鍵字並獲得步兵關鍵字。

**戰鬥程序。** 遊戲開始時，本模型所在的卡斯特蘭機器人單位啟動防護程序。在你的指揮階段，選擇以下一項新程序取代舊程序：衛士程序使卡斯特蘭機器人射擊武器攻擊次數加 2；侵攻程序使其近戰武器攻擊次數加 2；防護程序使其韌性加 1。

## 武器

| 武器 | 射程 | A | BS／WS | S | AP | D | 能力 |
|---|---:|---:|---:|---:|---:|---:|---|
| 機械教手槍 | 12 | 1 | 3+ | 6 | -1 | 1 | 毀滅傷害、手槍 |
| 動力拳 | 近戰 | 2 | 3+ | 8 | -2 | 2 | 無 |

## 編制、點數與裝備

| 項目 | 內容 |
|---|---|
| 編制 | 1 個智控數據技師。 |
| 點數 | 25 分 |
| 裝備 | 機械教手槍、動力拳。 |
| 升級選項 | 無。 |
| 版本註記 | 點數依官方 Munitorum Field Manual v1.2。 |

## 關鍵字

**關鍵字：** 步兵、人物、帝國、智控軍團、技術牧師、智控數據技師。

**陣營關鍵字：** 機械修會。

## 資料來源

本頁規則資料依使用者提供的《機械修會11版中文1.0.pdf》整理，並套用官方 Faction Pack 1.1 更新；點數依官方 Munitorum Field Manual v1.2。

## References

[1]: https://mfm.warhammer-community.com/en/adeptus-mechanicus "Munitorum Field Manual｜Adeptus Mechanicus v1.2"
[2]: https://assets.warhammer-community.com/eng_22-07_warhammer_40,000_faction_pack_adeptus_mechanicus-d1ubc1apog-mpt3r8xzy4.pdf "Faction Pack: Adeptus Mechanicus Version 1.1"
