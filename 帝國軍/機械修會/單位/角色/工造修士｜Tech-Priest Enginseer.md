---
標題: 工造修士
英文名稱: Tech-Priest Enginseer
陣營: 機械修會
來源版本: 使用者提供《機械修會11版中文1.0.pdf》；Faction Pack 1.1；Munitorum Field Manual v1.2
更新日期: 2026-08-19
---

# 工造修士（Tech-Priest Enginseer）
## 官方產品圖片

![工造修士｜Tech-Priest Enginseer｜Games Workshop 官方商品圖](../../../../assets/官方商店/機械修會/Astra-Militarum-Tech-Priest-Enginseer.jpg)

> **圖片來源：** [Games Workshop 官方商店](https://www.warhammer.com/en-WW/shop/Astra-Militarum-Tech-Priest-Enginseer)。官方商品主圖。
> **圖片擷取日期：** 2026-08-22。


本頁依使用者提供的《機械修會11版中文1.0.pdf》建立，並以官方 Munitorum Field Manual v1.2 的現行點數為準。規則關鍵字採繁體中文；英文名稱僅保留作為原文索引。

## 單位數據

| M | T | SV | W | LD | OC | 特殊保護 |
|---:|---:|---:|---:|---:|---:|---:|
| 6 | 4 | 3+ | 3 | 7+ | 1 | 5+ |

## 能力

**陣營：機神律令。** 本模型可受機神律令影響。

**核心：領袖。** 本單位可加入雷鳴宗電僧、法身宗電僧、武裝奴工突破者、武裝奴工毀滅者、護教軍遊獵兵或護教軍先鋒兵。

**工造修士。** 如果本模型 3 吋內有至少一個友軍機械修會載具單位，本模型獲得獨行特工。

**機神祝福。** 在你的指揮階段，選擇本模型 3 吋內一個友軍機械修會模型，該模型恢復 D3 點失去的傷口；若該模型是載具，直到下一個你的指揮階段開始獲得不知疼痛 5+。每個模型每個指揮階段只能被選擇一次。

**機神復仇。** 若友軍機械修會載具在本模型 12 吋內被消滅，本模型的機神戰斧在本場遊戲中攻擊次數變為 6。

## 武器

| 武器 | 射程 | A | BS／WS | S | AP | D | 能力 |
|---|---:|---:|---:|---:|---:|---:|---|
| 機械教手槍 | 12 | 1 | 3+ | 6 | -1 | 1 | 毀滅傷害、手槍 |
| 機神戰斧 | 近戰 | 3 | 4+ | 6 | -2 | 2 | 無 |
| 機械臂 | 近戰 | 1 | 4+ | 6 | -2 | 2 | 額外攻擊 |

## 編制、點數與裝備

| 項目 | 內容 |
|---|---|
| 編制 | 1 個工造修士。 |
| 點數 | 55 分 |
| 裝備 | 機械教手槍、機神戰斧、機械臂。 |
| 升級選項 | 無。 |
| 版本註記 | 點數依官方 Munitorum Field Manual v1.2。 |

## 關鍵字

**關鍵字：** 步兵、人物、帝國、機械教、技術牧師、工造修士。

**陣營關鍵字：** 機械修會。

## 資料來源

本頁規則資料依使用者提供的《機械修會11版中文1.0.pdf》整理，並套用官方 Faction Pack 1.1 更新；點數依官方 Munitorum Field Manual v1.2。

## References

[1]: https://mfm.warhammer-community.com/en/adeptus-mechanicus "Munitorum Field Manual｜Adeptus Mechanicus v1.2"
[2]: https://assets.warhammer-community.com/eng_22-07_warhammer_40,000_faction_pack_adeptus_mechanicus-d1ubc1apog-mpt3r8xzy4.pdf "Faction Pack: Adeptus Mechanicus Version 1.1"
