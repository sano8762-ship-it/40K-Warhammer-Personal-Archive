---
標題: 武裝奴工突破者
英文名稱: Kataphron Breachers
陣營: 機械修會
來源版本: 使用者提供《機械修會11版中文1.0.pdf》；Faction Pack 1.1；Munitorum Field Manual v1.2
更新日期: 2026-08-19
---

# 武裝奴工突破者（Kataphron Breachers）
## 官方產品圖片

![武裝奴工突破者｜Kataphron Breachers｜Games Workshop 官方商品圖](../../../../assets/官方商店/機械修會/Kataphron-Battle-Servitors-Breachers-2017.jpg)

> **圖片來源：** [Games Workshop 官方商店](https://www.warhammer.com/en-WW/shop/Kataphron-Battle-Servitors-Breachers-2017)。官方商品主圖。
> **圖片擷取日期：** 2026-08-22。


本頁依使用者提供的《機械修會11版中文1.0.pdf》建立，並以官方 Munitorum Field Manual v1.2 的現行點數為準。規則關鍵字採繁體中文；英文名稱僅保留作為原文索引。

## 單位數據

| M | T | SV | W | LD | OC | 特殊保護 |
|---:|---:|---:|---:|---:|---:|---:|
| 5 | 7 | 3+ | 3 | 7+ | 1 | 6+ |

## 能力

**陣營：機神律令。** 本單位可受機神律令影響。

**突破命令。** 本單位模型可重擲攻擊命中結果中的 1；若本單位位於機械修會戰線友軍 6 吋內，改為可重擲命中結果。

## 武器

| 武器 | 射程 | A | BS／WS | S | AP | D | 能力 |
|---|---:|---:|---:|---:|---:|---:|---|
| 重型電弧步槍 | 30 | 2 | 4+ | 8 | -2 | 3 | 反載具 4+、速射 2 |
| 扭曲炮 | 36 | D3 | 4+ | 6 | -2 | 2 | 反步兵 2+、爆炸 |
| 電弧爪 | 近戰 | 4 | 4+ | 5 | -1 | 1 | 反載具 4+ |
| 液壓鉗 | 近戰 | 2 | 4+ | 8 | -2 | 3 | 無 |

## 編制、點數與裝備

| 項目 | 內容 |
|---|---|
| 編制 | 3 個武裝奴工突破者；可增加 3 個相同模型。 |
| 點數 | 150／310 分 |
| 裝備 | 重型電弧步槍、電弧爪。 |
| 升級選項 | 任意模型可將重型電弧步槍換為扭曲炮；任意模型可將電弧爪換為液壓鉗。 |
| 版本註記 | MFM v1.2：3／6 模型為 150／310 分；點數依官方 Munitorum Field Manual v1.2。 |

## 關鍵字

**關鍵字：** 步兵、帝國、機械教、武裝奴工、突破者。

**陣營關鍵字：** 機械修會。

## 資料來源

本頁規則資料依使用者提供的《機械修會11版中文1.0.pdf》整理，並套用官方 Faction Pack 1.1 更新；點數依官方 Munitorum Field Manual v1.2。

## References

[1]: https://mfm.warhammer-community.com/en/adeptus-mechanicus "Munitorum Field Manual｜Adeptus Mechanicus v1.2"
[2]: https://assets.warhammer-community.com/eng_22-07_warhammer_40,000_faction_pack_adeptus_mechanicus-d1ubc1apog-mpt3r8xzy4.pdf "Faction Pack: Adeptus Mechanicus Version 1.1"
