---
標題: 天蠍座粉碎者坦克
英文名稱: Skorpius Disintegrator
陣營: 機械修會
來源版本: 使用者提供《機械修會11版中文1.0.pdf》；Faction Pack 1.1；Munitorum Field Manual v1.2
更新日期: 2026-08-19
---

# 天蠍座粉碎者坦克（Skorpius Disintegrator）
## 官方產品圖片

![天蠍座粉碎者坦克｜Skorpius Disintegrator｜Games Workshop 官方商品圖](../../../../assets/官方商店/機械修會/Adeptus-Mechanicus-Skorpius-Disintegrator-2019.jpg)

> **圖片來源：** [Games Workshop 官方商店](https://www.warhammer.com/en-WW/shop/Adeptus-Mechanicus-Skorpius-Disintegrator-2019)。官方商品主圖。
> **圖片擷取日期：** 2026-08-22。


本頁依使用者提供的《機械修會11版中文1.0.pdf》建立，並以官方 Munitorum Field Manual v1.2 的現行點數為準。規則關鍵字採繁體中文；英文名稱僅保留作為原文索引。

## 單位數據

| M | T | SV | W | LD | OC | 特殊保護 |
|---:|---:|---:|---:|---:|---:|---:|
| 10 | 10 | 2+ | 12 | 7+ | 3 | — |

## 能力

**陣營：機神律令。** 本模型可受機神律令影響。

**核心：致命破滅 D3。** 本模型擁有致命破滅 D3。

**廣域數據鏈。** 本模型成為你的策略目標時，擲 D6；5+ 時你獲得 1 CP。

**爆裂齊射。** 使用拜爾羅斯能量炮攻擊步兵時命中結果加 1；使用鐵融炮攻擊巨獸或載具時命中結果加 1。

**嚴重損傷。** 本模型傷口為 1–4 時，攻擊命中結果減 1。

## 武器

| 武器 | 射程 | A | BS／WS | S | AP | D | 能力 |
|---|---:|---:|---:|---:|---:|---:|---|
| 拜爾羅斯能量炮 | 36 | 2D6 | 4+ | 7 | -2 | 1 | 爆炸、曲射 |
| 智能重機槍 | 36 | 3 | 4+ | 4 | 0 | 1 | 速射 3、連擊 1 |
| 瓦解者導彈 | 36 | 3 | 4+ | 9 | -2 | D6 | 雙聯 |
| 鐵融炮 | 48 | 3 | 4+ | 12 | -3 | D6+1 | 無 |
| 裝甲車體 | 近戰 | 3 | 4+ | 6 | 0 | 1 | 無 |

## 編制、點數與裝備

| 項目 | 內容 |
|---|---|
| 編制 | 1 臺天蠍座粉碎者坦克。 |
| 點數 | 160 分 |
| 裝備 | 拜爾羅斯能量炮、3 挺智能重機槍、瓦解者導彈、裝甲車體。 |
| 升級選項 | 拜爾羅斯能量炮可換為鐵融炮，+10 分。 |
| 版本註記 | Faction Pack 1.1 已加入模型框架關鍵字；點數依 MFM v1.2：160 分 |

## 關鍵字

**關鍵字：** 載具、帝國、護教軍、煙霧彈、模型框架、天蠍座粉碎者。

**陣營關鍵字：** 機械修會。

## 資料來源

本頁規則資料依使用者提供的《機械修會11版中文1.0.pdf》整理，並套用官方 Faction Pack 1.1 更新；點數依官方 Munitorum Field Manual v1.2。

## References

[1]: https://mfm.warhammer-community.com/en/adeptus-mechanicus "Munitorum Field Manual｜Adeptus Mechanicus v1.2"
[2]: https://assets.warhammer-community.com/eng_22-07_warhammer_40,000_faction_pack_adeptus_mechanicus-d1ubc1apog-mpt3r8xzy4.pdf "Faction Pack: Adeptus Mechanicus Version 1.1"
